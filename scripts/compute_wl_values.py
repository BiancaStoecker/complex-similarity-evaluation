...
def calculate_wl_values(pair):
    """ Calculation of Weisfeiler-Lehman values for 0, 1 and 2 iterations for the
        given graphs.
    """
    try:
        output = check_output("java -jar tools/wljaccard/wljaccard.jar --iter 3 {} {} ".format(prefix_gml+pair[0], prefix_gml+pair[1]), shell=True)
        wl0, wl1, wl2, _ = output.decode("utf8").strip("\n").split("\t")
        return (pair[0], pair[1], pair[2], wl0, wl1, wl2, None)
...
if __name__ == "__main__":
    ...
    with open(snakemake.output[0], "wt") as logfile:
        logfile.write("\t".join(["c1", "c2", "edit_sim", "wl0", "wl1", "wl2"])+"\n")
        for c1, c2, edit_sim, wl0, wl1, wl2, error in p.imap(calculate_wl_values, pairs): # provide pairs
            if error is None:
                output = "\t".join([c1, c2, str(edit_sim), wl0, wl1, wl2])
